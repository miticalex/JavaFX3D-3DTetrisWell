...
public class Edges extends Group{
    ...
    public Edges(int width, int height, double edgeThickness, double fieldSize) {
        ...
        for (int i = -1; i < 2*width+1; i++) {
            ...
            rearEdge.setTranslateY(fieldSize*(-0.5*height - 0.24)); // addNodeToGroupGridXYZ(edges, rearEdge, 0.5*i - 0.25, -0.75, -0.5);
            
            Box frontEdge = new Box(fieldSize/2, fieldSize/2, edgeThickness);
            frontEdge.setTranslateX(fieldSize*(-0.5*width + 0.25 + 0.5*i));
            frontEdge.setTranslateY(fieldSize*(0.5*height + 0.24)); // addNodeToGroupGridXYZ(edges, frontEdge, 0.5*i - 0.25, height - 0.25, -0.5);
        ...
        for (int i = 0; i < 2*height; i++) {
            ...
            leftEdge.setTranslateX(fieldSize*(-0.5*width - 0.24));
            ...
            rightEdge.setTranslateX(fieldSize*(0.5*width + 0.24));
